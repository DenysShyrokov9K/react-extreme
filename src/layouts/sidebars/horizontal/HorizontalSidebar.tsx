...
const HorizontalSidebar = () => {
  const activeBg = useSelector((state : StateType) => state.customizer.sidebarBg);
  const location = useLocation();
  const currentURL = location.pathname.split('/').slice(0, -1).join('/');
  const isFixed = useSelector((state : StateType) => state.customizer.isSidebarFixed);
  const isMobileSidebar = useSelector((state : StateType) => state.customizer.isMobileSidebar);
  return (
    <div
